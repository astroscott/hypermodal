class Osc {
  float a, a_peak, f, f_rot, r;
  float ca = 0; // "counter amplitude" (counter used for cyclic oscillation animation).
  float co = 0; // "counter offset" (counter used for cyclic rotation animation).
  int fr, n, nn, k;
  int[] c = new int[3];
  float[] pos;
  Node[] nodes;

  Osc(float peak_amplitude, float frequency, float rotational_frequency, int num_peaks, int num_nodes, float radius, int framerate) {
    a = 0;
    a_peak = peak_amplitude;       // [  ] Peak amplitude of oscillator
